...
###############################################################################
if [ "$WPLT" = "-wp" ]; then
    for cat in 'Artist' 'Album' 'Song'
    do
        printf "${RED}* BarChart Plots [${cat}]...${NCL}\n"
        python Plot_HistoricPlot.py $USRM "$PTHO/data" "$PTHO/img" $cat
    done
fi
###############################################################################
# Wordclouds
...
if [ "$WPLT" = "-wp" ]; then
    ...
    do
        yhi=$(expr $ylo + 1)
        printf "${RED}* Clock Plots [${ylo}:${yhi}]...${NCL}\n"
        python Plot_PolarClocks.py $USRM "$PTHO/data" "$PTHO/img" "$PTHO/fonts" "$ylo" "$yhi"
    done
fi
###############################################################################
# Strip Plots
